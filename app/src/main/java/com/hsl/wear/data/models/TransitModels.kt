...
data class NotificationPreferences(
    val preArrivalEnabled: Boolean = true,
    val vibrationOnly: Boolean = false,           // false = vibration + on-screen
    val advanceMinutes: Int = 2,                  // 2, 3, or 5 minutes before arrival
    val vibrationIntensity: VibrationIntensity = VibrationIntensity.MEDIUM,
    val transferNotificationsEnabled: Boolean = true,  // notify before transfers
    val finalDestinationNotificationsEnabled: Boolean = true, // notify before final destination
    val walkingNotificationsEnabled: Boolean = false   // notify during walking phases
)
...
data class NotificationState(
    val lastTransferNotificationTime: Long? = null,      // Timestamp of last transfer notification
    val lastDestinationNotificationTime: Long? = null,    // Timestamp of last destination notification
    val currentLegIndex: Int = 0,                         // Current leg index in route
    val nextTransferLegIndex: Int? = null,                 // Index of next transfer leg
    val isApproachingTransfer: Boolean = false,           // Within notification window for transfer
    val isApproachingDestination: Boolean = false        // Within notification window for final destination
)
